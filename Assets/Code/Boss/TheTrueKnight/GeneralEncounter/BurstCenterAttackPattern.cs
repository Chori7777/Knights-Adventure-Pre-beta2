...
public class BurstCenterAttackPattern : MonoBehaviour, IAttackPattern
{
    ...
    private IEnumerator FireBurst()
    {
        ...
        if (spawn == null || projectilePrefab == null)
        {
            OnFinished?.Invoke();
            yield break;
        }
        ...
        if (player != null)
            baseDir = ((Vector2)player.position - (Vector2)spawn.position).normalized;
        ...
        for (int i = 0; i < spawnCount; i++)
        {
            ...
            GameObject p = Instantiate(projectilePrefab, spawn.position, Quaternion.identity);
